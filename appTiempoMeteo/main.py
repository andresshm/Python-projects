...
sg.theme('reddit')
imagen_col = sg.Column([[sg.Image(key='-IMAGE-', background_color='#FFFFFF')]])
info_col = sg.Column([
...
layout = [
    [sg.Input(expand_x=True, key='-INPUT-'), sg.Button('Enter', button_color='Black', border_width=0)],
    [imagen_col, info_col]
]
...
while True:
    ...
    if event == 'Enter':
        name, time, weather, temp = get_weather_location(values['-INPUT-'])
        get_weather_location(values['-INPUT-'])
        ...
        if weather in ('Sun', 'Sunny'):
            pass # imprimir una foto de un sol
        #si llueve imprimir foto de lluvia...
        window['-IMAGE-'].update('imagenes/nube.png')
